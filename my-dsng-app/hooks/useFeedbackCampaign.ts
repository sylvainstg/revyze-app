...
import { FeedbackCampaign, RemoteConfigCampaign, FeedbackAnswer } from '../types/campaign';
import {
    getActiveCampaigns,
    selectVariant,
    checkFrequencyCap,
...
export const useFeedbackCampaign = (currentUser: User | null): UseFeedbackCampaignResult => {
    ...
    const fetchActiveCampaign = async () => {
        ...
        try {
            ...
            for (const camp of campaigns) {
                ...
                if (dismissedAt && (now - dismissedAt) < DISMISS_DURATION_DAYS * 24 * 60 * 60 * 1000) {
                    continue;
                }

                // Check frequency cap
                const canShow = await checkFrequencyCap(currentUser.id, camp.frequencyCapDays);
                if (!canShow) {
                    continue;
                }
    ...
    const dismiss = () => {
        ...
        // Record dismissal
        if (currentUser) {
            recordCampaignDismissed(currentUser.id, activeCampaignId, campaign?.variant?.id);
        }
...
const getUserSegment = (user: User): string => {
    ...
    const accountAge = user.createdAt ? (Date.now() - new Date(user.createdAt).getTime()) / (1000 * 60 * 60 * 24) : 999;
    ...
    if ((plan === 'pro' || plan === 'business') && (daysSinceActive > 30 || engagementScore < 40)) {
        return 'at_risk_pros';
    }

    // Returning inactive users: Just logged in, but previous login was > 30 days ago
    const previousActive = user.previousLogin ? new Date(user.previousLogin).getTime() : 0;
    const daysSincePreviousActive = (Date.now() - previousActive) / (1000 * 60 * 60 * 24);
