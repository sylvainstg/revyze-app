...
export const useEngagement = (enabled: boolean): UseEngagementResult => {
  ...
  const debugMode =
    urlDebug === "1" || sessionStorage.getItem(DEBUG_STORAGE_KEY) === "1";
  const forceCampaignId =
    urlForceId || sessionStorage.getItem(FORCE_ID_STORAGE_KEY);

  const fetchActive = async () => {
    if (!enabled) {
      // Must wait for auth
      if (debugMode)
        console.log("[Engagement DEBUG] Not enabled (waiting for auth).");
      return;
    }
    ...
    try {
      ...
      if (debugMode)
        console.log(
          `[Engagement DEBUG] Fetching active campaign. ForceID: ${forceCampaignId || "none"}`,
        );
      ...
      if (!active) {
        if (debugMode)
          console.log("[Engagement DEBUG] No active campaign returned.");
        setCampaign(null);
        return;
      }
      ...
      if (debugMode) {
        console.log(
          "[Engagement DEBUG] Bypassing frequency caps and dismissals for:",
          active.id,
        );
        setCampaign(active);
        return;
      ...
      if (forceCampaignId && active.id !== forceCampaignId) {
        console.log(
          `[Engagement DEBUG] Forcing campaign ${forceCampaignId}, but got ${active.id}. Filtering out.`,
        );
        setCampaign(null);
        return;
      }
