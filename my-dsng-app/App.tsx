...
import {
  ...
  getNextCategoryVersion,
  DEFAULT_CATEGORY,
  migrateVersionsToCategories,
} from "./utils/categoryHelpers";
import {
  fetchStripePricing,
  enrichPlansWithPricing,
...
const App: React.FC = () => {
  ...
  const [activeCategory, setActiveCategory] =
    useState<string>(DEFAULT_CATEGORY); // Active document category
  ...
  const [categoryViewStates, setCategoryViewStates] = useState<
    Record<string, { scale: number; panOffset: { x: number; y: number } }>
  >({});
  ...
  const activeVersion =
    activeProject?.versions.find(
      (v) => v.id === activeProject.currentVersionId,
    ) ||
  ...
  useEffect(() => {
    ...
    const currentVersion = activeProject.versions.find(
      (v) => v.id === activeProject.currentVersionId,
    );
  ...
  const {
    ...
    submit: submitEngagementAnswer,
    dismiss: dismissEngagement,
  } = useEngagement(!!currentUser && !authLoading);
    ...
    const fetchData = async () => {
      try {
        ...
        try {
          ...
          console.error(
            "Failed to fetch plan limits, using defaults:",
            limitError,
          );
        }
        ...
        const dynamicMetadata = {
          ...
          business: {
            ...PLAN_METADATA.business,
            limits: currentLimits.business,
          },
        };
        ...
        console.warn(
          "Using plan metadata without Stripe pricing. Update functions/.env with production Stripe key.",
        );
        setPricingError(
          "Could not load pricing from Stripe. Using default prices.",
        );
        ...
        const fallbackPlans = {
          ...
          free: {
            ...PLAN_METADATA.free,
            limits: currentLimits.free,
            price: { monthly: "$0", yearly: "$0" },
            priceIds: { monthly: null, yearly: null },
          },
          pro: {
            ...PLAN_METADATA.pro,
            limits: currentLimits.pro,
  ...
  useEffect(() => {
    const loadProjects = async () => {
      if (currentUser && !isGuest) {
        const userProjects =
          await storageService.getProjectsForUser(currentUser);
        // Migrate versions to ensure they have category fields
        const migratedProjects = userProjects.map((project) => {
          const migratedVersions = migrateVersionsToCategories(
            project.versions,
          );
          // Check if migration actually changed anything
          const needsMigration = migratedVersions.some(
            (v, i) =>
              v.category !== project.versions[i]?.category ||
              v.categoryVersionNumber !==
              project.versions[i]?.categoryVersionNumber,
          );

          // If migration added fields, save to Firestore
          if (needsMigration) {
            console.log(
              "[Migration] Saving migrated versions to Firestore for project:",
              project.id,
            );
            storageService.updateProjectPartial(project.id, {
  ...
  useEffect(() => {
    ...
    const unsubscribe = storageService.subscribeToProject(
      activeProjectId,
      (updatedProject) => {
        console.log(
          "[Realtime Listener] Received update for project:",
          updatedProject.id,
        );
        ...
        if (updatedProject.deletedAt) {
          console.log(
            "[Realtime Listener] Project was deleted, removing from state",
          );
        ...
        console.log(
          "[Realtime Listener] Versions in update:",
          migratedProject.versions.map((v) => ({
            id: v.id,
            category: v.category,
            catVer: v.categoryVersionNumber,
          })),
        );

        // Ignore updates that come within 2 seconds of an optimistic update
        const timeSinceOptimistic =
          Date.now() - lastOptimisticUpdateRef.current;
        if (timeSinceOptimistic < 2000) {
          console.log(
            "[Realtime Listener] Ignoring update - too soon after optimistic update (",
            timeSinceOptimistic,
            "ms)",
          );
          return;
        ...
        setProjects((prev) => {
          ...
          if (current) {
            console.log(
              "[Realtime Listener] Current versions:",
          ...
          if (JSON.stringify(current) === JSON.stringify(migratedProject)) {
            console.log(
              "[Realtime Listener] No changes detected, skipping update",
            );
            return prev;
          }
          ...
          return prev.map((p) =>
            p.id === migratedProject.id ? migratedProject : p,
          );
        });
      },
    );
    ...
    if (projectIdParam) {
      ...
      if (targetProject) {
        ...
        const initialCategory = categories.includes(DEFAULT_CATEGORY)
          ? DEFAULT_CATEGORY
          : categories.find((c) => c !== "Mood Board") ||
          categories[0] ||
          DEFAULT_CATEGORY;
        ...
        console.warn(
          "[Deep Linking] Project not found or no access:",
          projectIdParam,
        );
        setPendingProjectId(null);
        const newUrl = window.location.origin + window.location.pathname;
  ...
  useEffect(() => {
    ...
    if (!isUserZooming.current) {
      console.log(
        "[Zoom Persistence] Skipping save - change not initiated by user",
      );
      return;
    }
  ...
  const updateProjectState = async (updatedProject: Project) => {
    ...
    if (!success) {
      alert(
        "Warning: Changes could not be saved to the cloud. Please check your connection.",
      );
  ...
  const handleCaptureThumbnail = async () => {
    ...
    if (!currentUser || activeProject.ownerId !== currentUser.id) {
      setToast({
        message: "Only the project owner can set the thumbnail.",
        type: "error",
      });
      return;
    }
    ...
    try {
      // Detect file type
      const activeVer = activeProject.versions.find(
        (v) => v.id === activeProject.currentVersionId,
      );
      if (!activeVer) return;
      ...
      if (isPDF) {
        ...
        if (!canvas) {
          setToast({
            message: "Could not capture thumbnail. Please try again.",
            type: "error",
          });
          return;
        }
        ...
        if (!moodBoardContainer) {
          setToast({
            message:
              "Could not capture thumbnail. Mood board container not found.",
            type: "error",
          });
          return;
        }
      ...
      const storageRef = ref(
        storage,
        `thumbnails/${activeProject.id}_${Date.now()}.jpg`,
      );
  ...
  const confirmDeleteProject = async () => {
    ...
    const success = await storageService.deleteProject(
      projectToDelete.id,
      currentUser.id,
    );
    if (success) {
  ...
  const handleRestoreProject = (project: Project) => {
    ...
    setProjects((prev) =>
      [...prev, project].sort((a, b) => b.lastModified - a.lastModified),
    );
  ...
  useEffect(() => {
    console.log(
      "[projectToDelete state changed]:",
      projectToDelete?.name || "null",
      "view:",
      view,
    );
  ...
  const handleSetDefaultPage = async (page: number) => {
    ...
    await updateProjectState(updatedProject);
    setToast({
      message: `Page ${page} set as default for ${activeCategory}`,
      type: "success",
    });
  };
  ...
  const handleAuthSuccess = (user: User, isNewUser: boolean) => {
    ...
    if (projectId && !token) {
      console.log(
        "[Auth] Deep link detected, letting deep link effect handle it",
      );
  ...
  const handleCheckoutSuccess = async (planId: string) => {
    try {
      ...
      const validPlan =
        planId === "free" || planId === "pro" || planId === "business"
          ? planId
          : "free";
      ...
      await storageService.updateUserPlan(
        currentUser.id,
        validPlan,
        subscriptionStatus,
      );
  ...
  const handleCreateProjectWithData = async (
    ...
    const isOwnedUnlimited =
      limits.ownedProjects === -1 || limits.ownedProjects === Infinity;
    if (!isOwnedUnlimited && ownedCount >= limits.ownedProjects) {
      if (
        confirm(
          `You have reached the limit of ${limits.ownedProjects} owned project${limits.ownedProjects > 1 ? "s" : ""} for the ${plan} plan. Upgrade to create more?`,
        )
      ) {
        setView("pricing");
      }
    ...
    const isTotalUnlimited =
      limits.totalProjects === -1 || limits.totalProjects === Infinity;
    if (!isTotalUnlimited && projects.length >= limits.totalProjects) {
      if (
        confirm(
          `You have reached the limit of ${limits.totalProjects} total projects for the ${plan} plan. Upgrade for unlimited projects?`,
        )
      ) {
        setView("pricing");
      }
    ...
    if (fileSizeMB > MAX_FILE_SIZE_MB) {
      alert(
        `File is too large (${fileSizeMB.toFixed(1)}MB). Please choose a file under ${MAX_FILE_SIZE_MB} MB.`,
      );
      return;
    }
    ...
    reader.onload = async (e) => {
      try {
        ...
        if (
          ...
          !project.collaborators.includes(currentUser.email)
        ) {
  ...
  const handleOpenProject = (id: string) => {
    ...
    // Set active category
    const projectActiveCategory =
      project.activeCategory ||
      getCategories(project.versions)[0] ||
      DEFAULT_CATEGORY;
    ...
    const defaultPage =
      project.categorySettings?.[projectActiveCategory]?.defaultPage;
    if (defaultPage) {
  ...
  // Modal State
  const [shareModalProjectId, setShareModalProjectId] = useState<string | null>(
    null,
  );
  ...
  const [versionToEdit, setVersionToEdit] = useState<ProjectVersion | null>(
    null,
  );
  ...
  const handleSaveProjectSettings = async (updates: {
    ...
    if (!projectSettingsProject) return;
    await storageService.updateProjectPartial(
      projectSettingsProject.id,
      updates,
    );
    setProjects((prev) =>
      prev.map((p) =>
        p.id === projectSettingsProject.id ? { ...p, ...updates } : p,
      ),
    );
  ...
  const handleUploadNewVersion = async (file: File, category: string) => {
    ...
    if (fileSizeMB > MAX_FILE_SIZE_MB) {
      setToast({
        message: `File is too large(${fileSizeMB.toFixed(1)}MB).Please choose a file under ${MAX_FILE_SIZE_MB} MB.`,
        type: "error",
      });
      return;
    }
    ...
    try {
      ...
      const storageRef = ref(
        storage,
        `projects / ${activeProject.id}/v${globalVersionNumber}_${file.name}`,
      );
      ...
      setToast({
        message: `Version ${categoryVersionNumber} uploaded to ${category}!`,
        type: "success",
      });
    } catch (error) {
      console.error("Error uploading new version:", error);
      setToast({
        message: "Failed to upload new version. Please try again.",
        type: "error",
      });
    }
  };
  ...
  const handleUpdateVersion = async (updates: {
    name: string;
    category: string;
  }) => {
    ...
    const updatedVersions = activeProject.versions.map((v) =>
      v.id === versionToEdit.id ? updatedVersion : v,
    );
    ...
    const success = await storageService.updateProjectPartial(
      activeProject.id,
      {
        versions: updatedVersions,
  ...
  const handleDeleteVersion = async () => {
    ...
    if (activeProject.versions.length <= 1) {
      setToast({
        message: "Cannot delete the only version of a project",
        type: "error",
      });
      return;
    }
    ...
    const updatedVersions = activeProject.versions.filter(
    ...
    if (versionIdToDelete === activeProject.currentVersionId) {
      ...
      const sameCategoryVersions = updatedVersions.filter(
        (v) => v.category === categoryOfDeleted,
      );
      if (sameCategoryVersions.length > 0) {
        newActiveVersionId = sameCategoryVersions[0].id;
      } else {
        // Find latest version overall
        const sorted = [...updatedVersions].sort(
  ...
  const getUnresolvedCommentsCount = (): number => {
    ...
    return activeVersion.comments.filter((c) => !c.resolved && !c.deleted)
      .length;
  };

  // Derived project for modal
  const projectToShare =
    projects.find((p) => p.id === shareModalProjectId) || null;
  ...
  const handleInviteUser = async (
    ...
    console.log(
      "[handleInviteUser] Optimistic update - new collaborators:",
      optimisticProject.collaborators,
    );
    setProjects((prev) =>
      prev.map((p) => (p.id === projectToShare.id ? optimisticProject : p)),
    );

    // Persist to Firestore
    console.log("[handleInviteUser] Calling storageService.addCollaborator");
    const updatedProject = await storageService.addCollaborator(
      projectToShare.id,
      email,
    );
    console.log(
      "[handleInviteUser] Result from addCollaborator:",
      updatedProject?.id,
    );
    if (updatedProject) {
      setProjects((prev) =>
        prev.map((p) => (p.id === projectToShare.id ? updatedProject : p)),
      );
      ...
      try {
        ...
        setToast({
          message: `${email} added (email notification failed)`,
          type: "info",
        });
      }
    } else {
  ...
  const handleRemoveCollaborator = async (email: string) => {
    console.log(
      "[handleRemoveCollaborator] START - email:",
      email,
      "projectToShare:",
      projectToShare?.id,
    );
    ...
    const optimisticProject = {
      ...projectToShare,
      collaborators: (projectToShare.collaborators || []).filter(
        (c) => c !== email,
      ),
    };
    console.log(
      "[handleRemoveCollaborator] Optimistic update - remaining collaborators:",
      optimisticProject.collaborators,
    );
    setProjects((prev) =>
      prev.map((p) => (p.id === projectToShare.id ? optimisticProject : p)),
    ...
    console.log(
      "[handleRemoveCollaborator] Calling storageService.removeCollaborator",
    );
    const updatedProject = await storageService.removeCollaborator(
      projectToShare.id,
      email,
    );
    console.log(
      "[handleRemoveCollaborator] Result from removeCollaborator:",
      updatedProject?.id,
    );
    if (updatedProject) {
      setProjects((prev) =>
  ...
  const handleRevokeAll = async () => {
    ...
    const updatedProject = await storageService.resetProjectAccess(
      projectToShare.id,
    );
    if (updatedProject) {
      setProjects((prev) =>
  ...
  const handleUpdateShareSettings = async (
    ...
    const success = await storageService.updateProjectShareSettings(
      projectId,
      settings,
    );
    if (!success) {
  ...
  const handleUpdateCommentPosition = async (
    commentId: string,
    position: { x: number; y: number },
  ) => {
    if (!activeProject || !activeVersion || !currentUser) return;

    // Update Local Optimistically and Save to DB
    const updatedVersion = {
      ...activeVersion,
    ...
    const updatedProject = {
      ...
      versions: activeProject.versions.map((v) =>
        v.id === activeProject.currentVersionId ? updatedVersion : v,
      ),
    };
  ...
  const handleAddComment = async (
    newCommentData: Omit<Comment, "id" | "timestamp" | "resolved" | "audience">,
  ) => {
    if (!activeProject || !activeVersion || !currentUser) return;
  ...
  const handleResolveComment = async (id: string) => {
    ...
    if (activeProject.ownerId !== currentUser.id) {
      console.warn("[App.tsx] Unauthorized resolve attempt:", {
        id,
  ...
  const handleDeleteComment = async (id: string) => {
    ...
    if (!isOwner && !isAuthor) {
      console.warn("[App.tsx] Unauthorized delete attempt:", {
        id,
        userId: currentUser.id,
      });
      return;
  ...
  const handleUpdateMoodBoardElements = async (
    ...
    const updatedProject = {
      ...activeProject,
      versions: activeProject.versions.map((v) =>
        v.id === activeVersion.id ? { ...v, moodBoardElements: elements } : v,
      ),
    };
  ...
  if (view === "admin" && currentUser) {
    return (
      <div className="h-screen bg-white overflow-auto">
        <AdminPage onBack={() => setView("dashboard")} currentUser={currentUser} />
      </div>
    );
  }
      ...
      <>
        ...
        <ShareModal
          ...
          onTransferOwnership={async (newOwnerEmail) => {
            ...
            try {
              const result = await storageService.transferProjectOwnership(
                projectToShare.id,
                currentUser.id,
                newOwnerEmail,
              );
        ...
        <div className="h-screen flex flex-col items-center justify-center bg-slate-50">
          <div className="text-center p-8 max-w-md">
            ...
            <p className="text-slate-600 mb-6">
              The active version of this project could not be loaded. It might
              have been deleted or the data is corrupted.
            </p>
    ...
    return (
      <React.Fragment>
        ...
        <div className="h-screen flex flex-col overflow-hidden bg-slate-50">
          {/* Header */}
          <header
            className="h-16 bg-white border-b border-slate-200 px-6 flex items-center justify-between shrink-0 z-30"
          ...
          {activeProject && (
            <CategorySelector
              ...
              onCategoryChange={(category) => {
                ...
                const categoryVersions = getVersionsByCategory(
                  activeProject.versions,
                  category,
                );
                if (categoryVersions.length > 0) {
                  ...
                  const defaultPage =
                    activeProject.categorySettings?.[category]?.defaultPage;
          ...
          <main className="flex-1 flex overflow-hidden">
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
              {(() => {
                ...
                const currentProjectRole = getProjectRole(
                  activeProject,
                  currentUser,
                  isGuest,
                  impersonatedRole,
                ...
                const filteredComments = activeVersion.comments.filter(
                  (c) => canSeeComment(c, currentProjectRole) && !c.deleted,
                );
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center bg-slate-50 p-8 text-center">
                      <div className="bg-white p-8 rounded-xl shadow-sm max-w-md w-full">
                        ...
                        <p className="text-slate-500 mb-6">
                          There are no documents in this category yet. Upload
                          the first version to get started.
                        </p>
                        <Button
                ...
                console.log("[DEBUG] Rendering workspace:", {
                  ...
                  onEditVersionDefined: !!(activeProject.ownerId ===
                    currentUser.id
                ...
                const sortedCategoryVersions = [...categoryVersions].sort(
                  (a, b) => b.timestamp - a.timestamp,
                );
                const latestVersionId =
                  sortedCategoryVersions.length > 0
                    ? sortedCategoryVersions[0].id
                    : null;
                    ...
                    {!isLatestVersion && (
                      <div className="bg-amber-50 border border-amber-200 text-amber-800 px-4 py-2 flex items-center justify-between text-sm">
                        <div className="flex items-center gap-2">
                          ...
                          <span>
                            You are viewing an older version. Editing and
                    ...
                    <div className="flex-1">
                      {activeCategory === "Mood Board" ? (
                        ...
                        <PDFWorkspace
                          ...
                          initialPanOffset={
                            categoryViewStates[activeCategory]?.panOffset || {
                              x: 0,
                              y: 0,
                            }
                          }
                          ...
                          canAddComment={
                            activeProject.shareSettings?.accessLevel !==
                            "view" || !isGuest
                          }
        ...
        <ShareModal
          ...
          onInvite={(email, role, name) => {
            console.log(
              "[ShareModal] onInvite called with email:",
              email,
              "role:",
              role,
              "name:",
          ...
          onRemoveCollaborator={(email) => {
            console.log(
              "[ShareModal] onRemoveCollaborator called with email:",
              email,
            );
          ...
          onUpdateShareSettings={(projectId, settings) => {
            console.log(
              "[ShareModal] onUpdateShareSettings called:",
              projectId,
              settings,
            );
        ...
        {showVersionUploadModal && activeProject && activeVersion && (
          <VersionUploadModal
            ...
            existingCategories={Array.from(
              new Set([
                ...getCategories(activeProject.versions),
                ...STANDARD_CATEGORIES,
                activeCategory,
              ]),
            ).sort()}
        ...
        {/* Admin Dashboard */}
        {console.log("App.tsx: Render Logic", { isAdminMode, hasUser: !!currentUser })}
