...
export const aggregateDailyStats = async (
  ...
  logs.forEach((log) => {
    if (log.metadata && log.metadata.role) {
      ...
      else if (role === "designer" || role === "architect")
        activeUsersByRole.pro++;
  ...
  uniqueUserRoles.forEach((role) => {
    ...
    else if (role === UserRole.DESIGNER || role === "architect")
      finalActiveRoles.pro++; // Assuming 'architect' maps to pro
...
export const recalculateAllUserScores = async () => {
  ...
  const promises = usersSnap.docs.map((doc) =>
    updateUserEngagementScore(doc.id),
  );
...
export const getAnalyticsStats = async (
  ...
  return snapshot.docs
    .map((doc) => doc.data() as DailyAnalyticsStats)
