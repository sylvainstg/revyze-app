...
export const getProjectsForUser = async (user: User): Promise<Project[]> => {
  try {
    ...
    const [ownerSnapshot, collabSnapshot] = await Promise.all([
      getDocs(ownerQuery),
      getDocs(collabQuery),
    ]);
...
export const saveProject = async (project: Project): Promise<boolean> => {
  try {
    ...
    if (isNew) {
      ...
      logEvent(project.ownerId, "create_project", {
      ...
      if (project.versions.length > oldVersions.length) {
        logEvent(project.ownerId, "upload_version", {
          projectId: project.id,
          versionId: project.versions[project.versions.length - 1].id,
        });
      }
    }
 ...
 */
export const resetProjectAccess = async (
  projectId: string,
): Promise<Project | null> => {
  try {
...
export const getSharedProject = async (
  ...
  try {
    ...
    if (
      ...
      if (
        !projectData.currentVersionId &&
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (projectData as any).activeVersionId
      ) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ...
      const hasValidVersion = projectData.versions?.some(
        (v) => v.id === projectData.currentVersionId,
      );
      if (
        !hasValidVersion &&
...
export const updateProjectZoom = async (
  projectId: string,
  zoomLevel: number,
): Promise<boolean> => {
  try {
...
export const updateProjectPartial = async (
  projectId: string,
...
export const targetUserWithCampaign = async (
  userId: string,
