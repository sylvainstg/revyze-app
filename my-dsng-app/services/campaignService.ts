 ...
 */
export const getActiveCampaigns = async (segmentType: string): Promise<FeedbackCampaign[]> => {
 ...
 */
export const selectVariant = (campaign: FeedbackCampaign): { id: string; question: string; choices?: string[] } | null => {
...
export const recordConversion = async (
    ...
    const updatePromises = snapshot.docs.map(async (attrDoc) => {
        ...
        const totalValue = conversions.reduce((sum: number, conv: any) => sum + (conv.value || 0), 0);
