...
export const getUserReferralCode = async (): Promise<string> => {
    try {
        const getUserReferralCodeFn = httpsCallable(functions, 'getUserReferralCode');
...
export const applyReferralCode = async (referralCode: string): Promise<{ success: boolean; message: string }> => {
    try {
        const applyReferralCodeFn = httpsCallable(functions, 'applyReferralCodeFunction');
        const result = await applyReferralCodeFn({ referralCode });
        return result.data as { success: boolean; message: string };
    } catch (error: any) {
        console.error("Error applying referral code:", error);
        return { success: false, message: error.message || 'Failed to apply referral code' };
    }
};
...
export const getReferralStats = async (): Promise<ReferralStats> => {
    try {
        const getReferralStatsFn = httpsCallable(functions, 'getReferralStatsFunction');
 ...
 */
export const redeemTokensForFeature = async (featureId: string): Promise<{ success: boolean; message: string; newBalance?: number }> => {
    try {
        const redeemTokensFn = httpsCallable(functions, 'redeemTokens');
        const result = await redeemTokensFn({ featureId });
        return result.data as { success: boolean; message: string; newBalance?: number };
    } catch (error: any) {
        console.error("Error redeeming tokens:", error);
        return { success: false, message: error.message || 'Failed to redeem tokens' };
    }
};
...
export const getAdminReferralData = async () => {
    try {
        const getAdminReferralDataFn = httpsCallable(functions, 'getAdminReferralData');
...
export const updateFeatureCost = async (
    ...
    try {
        ...
        return { success: false, message: error.message || 'Failed to update feature cost' };
    }
};
