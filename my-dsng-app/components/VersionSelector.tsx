      ...
      >
        {sortedVersions.map((version) => {
          ...
          const unresolvedCount =
            version.comments?.filter((c) => !c.resolved && !c.deleted).length ||
            0;

          return (
            <option key={version.id} value={version.id}>
              {version.name || `v${version.versionNumber}`} - {version.fileName}{" "}
...
export const VersionSelectorDetailed: React.FC<
  VersionSelectorDetailedProps
> = ({ versions, currentVersionId, onVersionChange, onEditVersion }) => {
  ...
  return (
    <div className="relative">
      <button
        ...
        title={
          currentVersion?.name || `Version ${currentVersion?.versionNumber}`
        }
      >
      ...
      {isOpen && (
        <>
          ...
          />
          <div
            className="absolute left-0 mt-2 w-[420px] bg-white rounded-xl shadow-lg border border-slate-200 py-2 max-h-96 overflow-y-auto"
            style={{ zIndex: 99999 }}
          >
            {sortedVersions.map((version) => {
              ...
              const commentsCount = version.comments?.length || 0;
              const unresolvedCount =
                version.comments?.filter((c) => !c.resolved && !c.deleted)
                  .length || 0;
                ...
                >
                  ...
                  >
                    <div className="flex items-center gap-2 mb-1">
                      <span
                        className={`font-semibold truncate ${isActive ? "text-indigo-600" : "text-slate-900"}`}
                      >
