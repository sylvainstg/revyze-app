...
export const NotificationSettings: React.FC<Props> = ({ user, onChange }) => {
  ...
  return (
    <div className="space-y-6">
            ...
            <div className="flex gap-2">
              {["instant", "daily", "weekly", "none"].map((option) => (
                <button
                  ...
                  className={`px-3 py-1.5 text-xs font-medium rounded-md capitalize transition-colors ${
                    preferences.projectUpdates === option
                      ? "bg-indigo-600 text-white shadow-sm"
                      : "bg-white text-slate-600 border border-slate-200 hover:bg-slate-100"
                  }`}
                >
      ...
      {(preferences.mentions === "daily" ||
        preferences.mentions === "weekly" ||
        preferences.projectUpdates === "daily" ||
        preferences.projectUpdates === "weekly") && (
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setDigest((prev) => ({ ...prev, dailyTime: e.target.value }))
                }
            ...
            <div className="col-span-2">
              ...
              <select
                ...
                onChange={(e) =>
                  setDigest((prev) => ({ ...prev, timezone: e.target.value }))
                }
                className="w-full px-2 py-1.5 text-sm border border-slate-300 rounded-md bg-white"
              >
                {[
