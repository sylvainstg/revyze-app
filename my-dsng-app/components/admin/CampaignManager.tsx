...
export const CampaignManager: React.FC<CampaignManagerProps> = ({ onBack }) => {
  const [campaigns, setCampaigns] = useState<
    (FeedbackCampaign & { analytics?: CampaignAnalytics })[]
  >([]);
  ...
  const loadCampaignAnalytics = async () => {
    try {
      ...
      users.forEach((user: any) => {
        ...
        const slowReturn =
          lastLogin > 0
            ...
            : false;
        const shortSession =
          lastSessionDuration > 0 ? lastSessionDuration < 3 * 60 * 1000 : false;
        ...
        if (
          ...
          segmentCounts["giving_up_almost"] =
            (segmentCounts["giving_up_almost"] || 0) + 1;
        } else if (score >= 70 && plan !== "free") {
          segmentCounts["power_users"] =
            (segmentCounts["power_users"] || 0) + 1;
        } else if (plan !== "free" && score < 40) {
          segmentCounts["at_risk_pros"] =
            (segmentCounts["at_risk_pros"] || 0) + 1;
          ...
          const wasInactive =
            lastLogin - previousLogin > 30 * 24 * 60 * 60 * 1000;
          if (recentlyActive && wasInactive) {
            segmentCounts["returning_inactive_users"] =
              (segmentCounts["returning_inactive_users"] || 0) + 1;
          }
        }
      });
      ...
      const segmentDistribution = segments.map((segment) => ({
        ...
        percentage:
          totalUsers > 0
            ? Math.round(((segmentCounts[segment] || 0) / totalUsers) * 100)
            : 0,
      }));

      // Fetch campaign attribution data
      const attributionsSnap = await getDocs(
        collection(db, "campaign_attribution"),
      );
      ...
      const campaignPerformance = campaigns.map((campaign) => {
        const campaignAttributions = attributions.filter(
          (a: any) => a.campaignId === campaign.id,
        );
        const shown = campaignAttributions.length;
        const dismissed = campaignAttributions.filter(
          (a: any) => a.status === "dismissed",
        ).length;
        const answered = campaignAttributions.filter(
        ...
        return {
          ...
          name:
            campaign.name || campaign.title || campaign.question || "Untitled",
          shown,
          dismissed,
          answered,
      ...
      result = result.filter((a) => {
        ...
        const answerText =
          typeof a.answer === "string"
            ...
            : JSON.stringify(a.answer).toLowerCase();
        return (
          name.includes(lowerTerm) ||
          email.includes(lowerTerm) ||
          answerText.includes(lowerTerm)
        );
      });
    }
    ...
    result.sort((a, b) => {
      ...
      switch (sortConfig.key) {
        ...
        case "answer":
          ...
          valB =
            typeof b.answer === "string" ? b.answer : JSON.stringify(b.answer);
          break;
        case "timestamp":
        default:
          valA = a.timestamp || 0;
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-slate-600 mb-6">
              Create your first feedback campaign to start collecting insights
            </p>
            <Button
              ...
              >
                <div className="p-6">
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <span
                          className={`px-2 py-0.5 rounded-full text-xs font-medium ${campaign.status === "active" ? "bg-green-100 text-green-800" : campaign.status === "paused" ? "bg-yellow-100 text-yellow-800" : campaign.status === "completed" ? "bg-slate-100 text-slate-800" : "bg-slate-100 text-slate-600"} `}
                        >
                          {campaign.status}
                        </span>
                      ...
                      <div className="flex gap-4 text-xs text-slate-500">
                        <span>
                          ...
                          <strong className="text-slate-700">
                            {getSegmentLabel(campaign)}
                          </strong>
                        </span>
                        <span>
                          Frequency:{" "}
                          <strong className="text-slate-700">
                            {campaign.frequencyCapDays ?? "—"}d
                          </strong>
                        </span>
                        <span>
                          Email Follow-up:{" "}
                          <strong className="text-slate-700">
                            {(campaign as any).emailFollowUpHours ??
                              (campaign as any).emailFallbackAfterHours ??
                              "—"}
                            h
                          </strong>
                        </span>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-5 gap-4 pt-4 border-t border-slate-100">
                      ...
                      <div>
                        <div className="text-2xl font-bold text-green-600">
                          {campaign.analytics.responseRate || 0}%
                        </div>
                        <div className="text-xs text-slate-500">
                          Response Rate
                        </div>
                      ...
                      <div>
                        <div className="text-2xl font-bold text-purple-600">
                          ${campaign.analytics.totalAttributedValue}
                        </div>
                        <div className="text-xs text-slate-500">
                          Attributed Value
                        </div>
                  ...
                  {campaign.variants &&
                    ...
                    campaign.analytics?.variantPerformance && (
                      <div className="mt-4 pt-4 border-t border-slate-100">
                        <h4 className="text-sm font-semibold text-slate-700 mb-3">
                          A/B Test Performance
                        </h4>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                          {campaign.analytics.variantPerformance.map(
                            (variant: any) => (
                              <div
                              ...
                              >
                                ...
                                <div className="flex gap-4 text-xs text-slate-600">
                                  ...
                                  <span>
                                    ${variant.averageValue.toFixed(2)} avg
                                  </span>
                                </div>
                              </div>
                            ),
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full p-6">
            ...
            <p className="text-slate-600 mb-6">
              Campaign creation UI coming soon. For now, use Firestore Console
              to create campaigns manually.
            </p>
            ...
            <div className="p-6 border-b border-slate-100 flex justify-between items-center gap-4">
              <div>
                ...
                <p className="text-sm text-slate-500">
                  {answersLoading
                    ? "Loading responses..."
                    : `${filteredAnswers.length} response(s) found`}
                </p>
              </div>
            ...
            <div className="p-6 overflow-y-auto">
              {answersLoading ? (
                ...
                <div className="text-center py-12 text-slate-500">
                  {searchTerm
                    ? "No matches found."
                    : "No responses recorded for this campaign yet."}
                </div>
              ) : (
                        ...
                        <td className="px-4 py-3 text-xs text-slate-500">
                          {ans.segmentData ? (
                            <div className="space-y-0.5">
                              <div>
                                Score: {ans.segmentData.engagementScore ?? "—"}
                              </div>
