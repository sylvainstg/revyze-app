...
export const PlanLimitsEditor: React.FC = () => {
    ...
    const loadLimits = async () => {
        ...
        try {
            ...
            const data = result.data as { limits: AllPlanLimits; source: 'firestore' | 'defaults' };
    ...
    const handleSave = async (planId: 'free' | 'pro' | 'business') => {
        ...
        try {
            ...
            setMessage({ text: `${planId.toUpperCase()} plan limits saved successfully`, type: 'success' });
            // Reload to confirm changes
            await loadLimits();
        } catch (error: any) {
            console.error('Failed to save plan limits:', error);
            setMessage({ text: error.message || 'Failed to save plan limits', type: 'error' });
        } finally {
            setSaving(false);
        }
    };
    ...
    const handleInitialize = async () => {
        if (!confirm('Initialize plan limits with defaults? This will overwrite any existing values.')) return;
        ...
        try {
            ...
            await initFunc();
            setMessage({ text: 'Plan limits initialized successfully', type: 'success' });
            await loadLimits();
        } catch (error: any) {
            console.error('Failed to initialize plan limits:', error);
            setMessage({ text: error.message || 'Failed to initialize plan limits', type: 'error' });
        } finally {
            setSaving(false);
        }
    };

    const updateLimit = (plan: 'free' | 'pro' | 'business', field: keyof PlanLimits, value: number) => {
        if (!limits) return;
        setLimits({
            ...limits,
            [plan]: {
            ...
            <div className="flex items-center justify-between py-2 border-b border-slate-100 last:border-0">
                ...
                <div className="flex items-center gap-2">
                    <input
                        type="number"
                        value={value}
                        onChange={(e) => updateLimit(plan, field, parseInt(e.target.value) || 0)}
                    ...
                    <span className="text-xs text-slate-500 w-24">
                        {value === -1 ? 'Unlimited' : field === 'storageMB' && value >= 1024 ? `${(value / 1024).toFixed(0)} GB` : value}
                    </span>
                </div>
            </div>
        );
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h3 className="text-lg font-bold text-slate-900">Plan Limits Configuration</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                    <div className="bg-slate-50 px-4 py-3 border-b border-slate-200">
                        ...
                        <p className="text-xs text-slate-500 mt-1">Basic tier for individuals</p>
                    </div>
                ...
                <div className="bg-white rounded-xl border border-indigo-200 shadow-sm overflow-hidden ring-2 ring-indigo-500">
                    <div className="bg-indigo-50 px-4 py-3 border-b border-indigo-200">
                        ...
                        <p className="text-xs text-indigo-600 mt-1">For professional designers</p>
                    </div>
                    <div className="p-4 space-y-1">
                ...
                <div className="bg-white rounded-xl border border-purple-200 shadow-sm overflow-hidden">
                    <div className="bg-purple-50 px-4 py-3 border-b border-purple-200">
                        ...
                        <p className="text-xs text-purple-600 mt-1">For teams and agencies</p>
                    </div>
                    <div className="p-4 space-y-1">
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <div className="text-sm text-blue-900">
                    <strong>Note:</strong> Use <code className="bg-blue-100 px-1 py-0.5 rounded">-1</code> to represent unlimited.
