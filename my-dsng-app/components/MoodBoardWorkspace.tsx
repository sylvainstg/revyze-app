...
export const MoodBoardWorkspace: React.FC<MoodBoardWorkspaceProps> = ({
    ...
    const updateElementContent = (id: string, content: string) => {
        onUpdateElements(elements.map(el => el.id === id ? { ...el, content } : el));
    };
        ...
        >
            ...
            >
                {elements.map(el => (
                    <div
                        key={el.id}
                        className={`absolute rounded-lg shadow-sm border-2 transition-shadow group ${selectedElementId === el.id ? 'border-indigo-500 ring-4 ring-indigo-200 shadow-xl' : 'border-white'
                            }`}
                        style={{
                            left: el.x,
                            top: el.y,
            ...
            <div className="absolute bottom-6 right-6 flex items-center gap-4">
                <div className="text-[10px] text-slate-400 font-bold bg-white/50 backdrop-blur-sm px-3 py-1.5 rounded-lg border border-slate-200/50 flex gap-4">
                    ...
                    <span className="flex items-center gap-1"><Move className="w-3 h-3" /> Alt + Drag to Pan</span>
                </div>
            </div>
        </div>
    );
