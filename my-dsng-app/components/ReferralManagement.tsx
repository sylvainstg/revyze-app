...
export const ReferralManagement: React.FC<Props> = ({ onClose }) => {
  ...
  const [editingFeature, setEditingFeature] =
    useState<Partial<FeatureCost> | null>(null);
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Get constants
      const getAdminReferralData = httpsCallable(
        functions,
        "getAdminReferralData",
      );
      const adminData = await getAdminReferralData({});
      const adminDataResult = adminData.data as {
  ...
  const handleSaveFeature = async () => {
    if (
      !editingFeature ||
      !editingFeature.id ||
      editingFeature.tokenCost === undefined
    ) {
    ...
    try {
      ...
      setMessage({
        text: "Feature cost updated successfully",
        type: "success",
      });
      ...
      setMessage({
        text: error.message || "Failed to save feature",
        type: "error",
      });
    }
  };
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div className="bg-white p-4 rounded-lg">
                    ...
                    <div className="text-2xl font-bold text-indigo-600">
                      {constants.tokensPerReferral}
                    </div>
                  </div>
                  <div className="bg-white p-4 rounded-lg">
                    <div className="text-sm text-slate-500">Expiration (Days)</div>
                    <div className="text-2xl font-bold text-indigo-600">
                      {constants.referralExpirationDays}
                    </div>
                  </div>
                  <div className="bg-white p-4 rounded-lg">
                    <div className="text-sm text-slate-500">Min. Redemption</div>
                    <div className="text-2xl font-bold text-indigo-600">
                      {constants.minimumRedemption}
                    </div>
                  </div>
                </div>
                <p className="text-xs text-indigo-700 mt-4">
                  To modify these constants, update REFERRAL_CONSTANTS in
                  functions/src/referralService.ts and redeploy.
                </p>
              </div>
            )}
                    ...
                    >
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-4 mt-2">
                          ...
                          <span
                            className={`px-2 py-0.5 rounded-full text-xs font-medium ${feature.enabled
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setEditingFeature({
                            ...editingFeature,
                            tokenCost: parseInt(e.target.value) || 0,
                          })
                        }
                        min="0"
                        className="bg-white text-slate-900"
