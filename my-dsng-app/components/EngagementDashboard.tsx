...
export const EngagementDashboard: React.FC<EngagementDashboardProps> = ({
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (stats.length > 0) {
        ...
        const avgDau =
          stats.reduce((acc, curr) => acc + curr.dau, 0) / stats.length;

        setSummary({
          mau: latest.mau || liveMau || 0,
          dau: Math.round(avgDau),
          stickiness:
            latest.mau > 0 ? Math.round((avgDau / latest.mau) * 100) : 0,
          liveness: 42,
          engagementScore: 68,
        });
  ...
  const chartData = data.map((d) => ({
    ...
    NewProjects: d.new_projects || 0,
    NewComments: d.new_comments || 0,
    NewInvites: d.new_invites || 0,
    ActivePros: d.active_users_by_role?.pro || 0,
    TotalUsers: d.mau || 0, // Proxy for total growth
  }));
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-slate-500 mb-6">
              Data will appear here as real usage events are collected.
            </p>
          </div>
        ) : (
          <>
            ...
            <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
              <h3 className="text-lg font-bold text-slate-900 mb-4">
                Role-Based Engagement Matrix (Last 7 Days)
              </h3>
