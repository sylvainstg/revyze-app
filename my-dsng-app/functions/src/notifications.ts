...
export const processNewMentions = async (
    ...
    for (const afterVer of afterVersions) {
        ...
        const newComments = afterComments.filter(c => !beforeComments.find(bc => bc.id === c.id));

        // Check for new replies in existing comments
        const existingComments = afterComments.filter(c => beforeComments.find(bc => bc.id === c.id));

        // Process new comments
        for (const comment of newComments) {
            if (comment.mentions && comment.mentions.length > 0) {
                await handleMentions(comment.mentions, comment, after.name, projectId, "comment");
            }
        }
        ...
        for (const comment of existingComments) {
            ...
            const newReplies = afterReplies.filter(r => !beforeReplies.find(br => br.id === r.id));
            ...
            for (const reply of newReplies) {
                if (reply.mentions && reply.mentions.length > 0) {
                    ...
                    await handleMentions(reply.mentions, reply, after.name, projectId, "reply", comment.text);
                }
            }
        }
    }
        ...
        try {
            ...
            if (!userSnap.empty) {
                ...
                if (prefs === 'none') {
                    ...
                    await queueForDigest(userId, email, type, item, projectName, projectId);
                }
