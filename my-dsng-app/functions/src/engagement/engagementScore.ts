 ...
 */
export const updateEngagementScore = functions.https.onCall(
  async (data, context) => {
    ...
    try {
      ...
      const sharePoints =
        ((userData.shareCountGuest || 0) + (userData.shareCountPro || 0)) * 20;
      ...
      console.log(`[Engagement] User ${userId}:`, {
        ...
        projectPoints,
        shareCount:
          (userData.shareCountGuest || 0) + (userData.shareCountPro || 0),
        sharePoints,
        baseScore: score,
      });
      ...
      console.log(
        `[Engagement] Activity points: ${activityPoints}, Total raw score: ${score}`,
      );
      ...
      const finalScore = Math.min(
        100,
        Math.round((score / MAX_RAW_SCORE) * 100),
      );

      console.log(
        `[Engagement] Final score: ${finalScore} (${score}/${MAX_RAW_SCORE})`,
      );
      ...
      throw new functions.https.HttpsError(
        "internal",
        error.message || "Failed to update engagement score",
      );
    }
  },
);
 ...
 */
export const recalculateAllEngagementScores = functions.https.onCall(
  async (data, context) => {
    // Verify admin
    if (!context.auth) {
      throw new functions.https.HttpsError(
