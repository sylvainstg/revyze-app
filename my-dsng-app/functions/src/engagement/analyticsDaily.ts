...
const reduceRole = (role?: string): keyof RoleBuckets => {
  ...
  if (normalized.includes("designer") || normalized.includes("architect"))
    return "pro";
...
export const computeDailyStats = async (targetDate: Date) => {
  ...
  console.log(
    `[analyticsDaily] computing stats for ${dateString} (${start.getTime()}-${end.getTime()})`,
  );
  ...
  const userIdsNeedingRole = Array.from(uniqueUsers).filter(
    (uid) => !userRoles.has(uid),
  );
  if (userIdsNeedingRole.length > 0) {
    const refs = userIdsNeedingRole.map((uid) =>
...
export const rebuildAnalyticsDaily = functions.https.onCall(
  async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        "unauthenticated",
    ...
    console.log(
      `[analyticsDaily] rebuild requested by ${context.auth.uid}, days=${days}`,
    );
    const results = [];
    ...
    try {
      ...
      console.log(`[analyticsDaily] rebuild complete by ${context.auth.uid}`, {
        daysComputed: computed.length,
      });
      ...
      throw new functions.https.HttpsError(
        "internal",
        error?.message || "Failed to rebuild daily analytics",
      );
    }
  },
);
...
export const rebuildAnalyticsDailyHttp = functions.https.onRequest(
  async (req, res) => {
    return corsHandler(req, res, async () => {
      ...
      try {
        ...
        console.log(
          `[analyticsDaily] HTTP rebuild requested by ${uid}, days=${days}`,
        );
        ...
        res.status(status).json({
          error: error?.message || "Failed to rebuild daily analytics",
        });
      }
    });
  },
