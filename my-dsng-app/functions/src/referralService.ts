...
export const applyReferralCode = async (
    ...
    try {
        ...
        const expiresAt = Date.now() + (REFERRAL_CONSTANTS.REFERRAL_EXPIRATION_DAYS * 24 * 60 * 60 * 1000);
        ...
        const referralRef = await admin.firestore().collection('referrals').add(referralData);
        ...
        console.log(`Referral created: ${referralRef.id} - ${referrerId} referred ${newUserId}`);
 ...
 */
export const processReferralConversion = async (convertedUserId: string): Promise<void> => {
    try {
        ...
        const referrerRef = admin.firestore().collection('users').doc(referralData.referrerId);
        ...
        console.log(`Referral converted: ${referralDoc.id} - Awarded ${tokensAwarded} tokens to ${referralData.referrerId}`);
    } catch (error) {
...
export const getUserReferralStats = async (userId: string): Promise<ReferralStats> => {
    try {
        ...
        const transactions = transactionsQuery.docs.map(doc =>
            ({ id: doc.id, ...doc.data() } as RewardTransaction)
        );
...
export const redeemTokensForFeature = async (
    ...
    try {
        // Get feature cost
        const featureDoc = await admin.firestore().collection('featureCosts').doc(featureId).get();
        ...
        if (!featureData?.enabled) {
            return { success: false, message: 'Feature is not available for redemption' };
        }
