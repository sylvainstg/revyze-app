    ...
    .onCreate(async (snap, context) => {
        ...
        try {
            ...
            await db.collection('scheduled_emails').add({
                ...
                userName: user?.name,
                scheduledFor: admin.firestore.Timestamp.fromMillis(Date.now() + delayMs),
                status: 'pending',
                createdAt: admin.firestore.FieldValue.serverTimestamp()
            });

            console.log(`Scheduled email follow-up for user ${userId}, campaign ${campaignId} in ${emailFollowUpHours} hours`);
            return null;
    ...
    .onRun(async (context) => {
        ...
        try {
            ...
            const promises = pendingEmails.docs.map(async (emailDoc) => {
                ...
                const { userId, campaignId, variantId, userEmail, userName } = emailData;

                try {
                    // Check if user has answered the campaign
                    const attributionQuery = await db.collection('campaign_attribution')
                        .where('userId', '==', userId)
                    ...
                    if (attributionQuery.empty) {
                        console.log(`No attribution found for user ${userId}, campaign ${campaignId}`);
                        await emailDoc.ref.update({ status: 'skipped', processedAt: admin.firestore.FieldValue.serverTimestamp() });
                        return;
                    }
                    ...
                    if (attribution.answeredAt) {
                        console.log(`User ${userId} already answered campaign ${campaignId}`);
                        await emailDoc.ref.update({ status: 'skipped', processedAt: admin.firestore.FieldValue.serverTimestamp() });
                        return;
                    }
                    ...
                    const campaignDoc = await db.collection('feedback_campaigns').doc(campaignId).get();
                    if (!campaignDoc.exists) {
                        await emailDoc.ref.update({ status: 'error', error: 'Campaign not found', processedAt: admin.firestore.FieldValue.serverTimestamp() });
                        return;
                    }

                    const campaign = campaignDoc.data();
                    const question = variantId && campaign?.variants
                    ...
                    console.log(`Sent follow-up email to ${userEmail} for campaign ${campaignId}`);
