...
export const createCampaign = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        if (!campaignData.name || !campaignData.question || !campaignData.type) {
            throw new functions.https.HttpsError('invalid-argument', 'Missing required fields: name, question, type');
        }
...
export const listCampaigns = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        // Get all campaigns
        const campaignsSnapshot = await db.collection('feedback_campaigns')
 ...
 * Callable Function
 */
export const getCampaignAnalyticsEndpoint = functions.https.onCall(async (data, context) => {
    // Check authentication
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'The function must be called while authenticated.');
    }
...
export const getSegmentStats = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        if (segmentType === 'power_users') {
            ...
            query = query.where('engagementScore', '>=', 70)
            ...
            throw new functions.https.HttpsError('invalid-argument', 'Unknown segment type');
        }
        ...
        if (segmentType === 'at_risk_pros') {
            ...
            users = users.filter(user => {
                ...
                const daysSincePreviousLogin = (now - previousLogin) / (1000 * 60 * 60 * 24);
 ...
 * Callable Function
 */
export const getUserCampaignHistory = functions.https.onCall(async (data, context) => {
    // Check authentication
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'The function must be called while authenticated.');
    }
    ...
    try {
        ...
        const attributionsSnapshot = await db.collection('campaign_attribution')
            .where('userId', '==', targetUserId)
            .orderBy('createdAt', 'desc')
            .get();

        // Fetch campaign details for each attribution to display names
        const history = await Promise.all(attributionsSnapshot.docs.map(async (doc) => {
            const attrData = doc.data();
            const campaignDoc = await db.collection('feedback_campaigns').doc(attrData.campaignId).get();
            const campaignData = campaignDoc.exists ? campaignDoc.data() : { name: 'Unknown Campaign', type: 'unknown' };
...
export const initSampleCampaign = functions.https.onRequest(async (req, res) => {
    ...
    const campaignData = {
        ...
        activeUntil: admin.firestore.Timestamp.fromMillis(Date.now() + 30 * 24 * 60 * 60 * 1000), // 30 days
        frequencyCapDays: 14,
...
export const listSampleCampaigns = functions.https.onRequest(async (req, res) => {
    // Simple security check
    if (req.query.key !== 'init_secret_123') {
        res.status(403).send('Unauthorized');
        return;
...
async function getCampaignAnalytics(campaignId: string) {
    ...
    const attributions = await db.collection('campaign_attribution')
        .where('campaignId', '==', campaignId)
        .get();

    const impressions = attributions.size;
    const responses = attributions.docs.filter(doc => doc.data().answeredAt).length;
    const responseRate = impressions > 0 ? (responses / impressions) * 100 : 0;

    // Get email metrics
    const emailsSent = await db.collection('user_activity')
        .where('eventName', '==', 'feedback_email_sent')
        .where('metadata.campaignId', '==', campaignId)
        .get();

    const emailsClicked = await db.collection('user_activity')
    ...
    const variantPerformanceArray = Object.values(variantPerformance).map((variant: any) => ({
        ...variant,
        responseRate: variant.impressions > 0 ? (variant.responses / variant.impressions) * 100 : 0,
        averageValue: variant.responses > 0 ? variant.totalValue / variant.responses : 0
    }));
    ...
    return {
        ...
        emailClickRate: emailsSent.size > 0 ? Math.round((emailsClicked.size / emailsSent.size) * 10000) / 100 : 0,
        totalConversions,
        totalAttributedValue,
        averageValuePerResponse: responses > 0 ? Math.round((totalAttributedValue / responses) * 100) / 100 : 0,
        variantPerformance: variantPerformanceArray.length > 0 ? variantPerformanceArray : undefined,
        updatedAt: new Date().toISOString()
    };
}
