...
const matchesSegmentQuery = (userData: FirebaseFirestore.DocumentData, query?: SegmentQuery): boolean => {
    ...
    switch (query.op) {
        case '<': return userValue < query.value;
        case '<=': return userValue <= query.value;
        case '==': return userValue == query.value; // eslint-disable-line eqeqeq
        case '>=': return userValue >= query.value;
        case '>': return userValue > query.value;
...
const buildCampaignPayload = (doc: FirebaseFirestore.DocumentSnapshot): FeedbackCampaign => {
...
export const feedbackActive = functions.https.onCall(async (data, context) => {
    ...
    const candidateCampaignsSnap = await db.collection('feedback_campaigns')
        .where('status', '==', 'active')
    ...
    const candidateCampaigns = candidateCampaignsSnap.docs.filter(doc => {
        const data = doc.data() as any;
        return data.activeUntil && data.activeUntil.toMillis ? data.activeUntil.toMillis() >= Date.now() : true;
    });
    ...
    for (const doc of candidateCampaigns) {
        ...
        if (campaign.segmentQuery?.field === 'segment' && campaign.segmentQuery.value === 'giving_up_almost') {
            ...
            const totalActions = (userData.projectCount || 0) + (userData.commentCount || 0) + (userData.shareCountGuest || 0) + (userData.shareCountPro || 0);

            const isNewish = createdAt > 0 ? (Date.now() - createdAt) <= 30 * 24 * 60 * 60 * 1000 : true;
            const hasReturned = loginCount >= 2;
            const isFree = plan === 'free';
            const slowReturn = lastLogin > 0 ? (Date.now() - lastLogin) > 2 * 24 * 60 * 60 * 1000 : false;
            const shortSession = lastSessionDuration > 0 ? lastSessionDuration < 180000 : false; // <3 min
            const someEngagement = totalActions >= 2;

            const qualifies = isNewish && isFree && hasReturned && (slowReturn || shortSession) && someEngagement;
...
export const adminCreateFeedbackCampaign = functions.https.onCall(async (data, context) => {
    ...
    if (!campaign.name || !campaign.question || !campaign.type || !campaign.activeFrom || !campaign.activeUntil) {
...
export const adminUpdateFeedbackCampaign = functions.https.onCall(async (data, context) => {
    const uid = requireAuth(context);
    await ensureAdmin(uid);
...
export const sendFeedbackEmailFallbacks = functions.https.onCall(async (data, context) => {
    const uid = requireAuth(context);
    await ensureAdmin(uid);

    const now = Date.now();
    const pending: FirebaseFirestore.QuerySnapshot = await db.collection('campaign_attribution')
    ...
    const tasks = pending.docs.map(async (doc) => {
        ...
        const campaign = buildCampaignPayload(campaignDoc);
        const fallbackMs = (campaign.emailFallbackAfterHours ?? 120) * 60 * 60 * 1000;
        const shownAt = attr.shownAt?.toDate ? attr.shownAt.toDate().getTime() : attr.shownAt;
        ...
        await doc.ref.update({ emailSentAt: admin.firestore.FieldValue.serverTimestamp() });
        await db.collection('user_activity').add({
            userId: attr.userId,
