...
import {
  generateDailyAnalyticsSnapshot,
  rebuildAnalyticsDaily,
  rebuildAnalyticsDailyHttp,
} from "./engagement/analyticsDaily";
import {
...
export const getUserPaymentHistory = functions.https.onCall(
  async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        "unauthenticated",
    ...
    if (!callerDoc.data()?.isAdmin) {
      throw new functions.https.HttpsError(
        "permission-denied",
        "Must be an admin to view payment history.",
      );
    }
...
export const getSubscriptionPlansFunction = functions
  .region("us-central1")
 ...
 */
export const getUserReferralCode = functions.https.onCall(
  async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        "unauthenticated",
 ...
 */
export const applyReferralCodeFunction = functions.https.onCall(
  async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        "unauthenticated",
 ...
 */
export const getReferralStatsFunction = functions.https.onCall(
  async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        "unauthenticated",
 ...
 */
export const getAdminReferralData = functions.https.onCall(
  async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        "unauthenticated",
    ...
    try {
      const referralsSnapshot = await admin
        .firestore()
 ...
 */
export const updateFeatureCost = functions.https.onCall(
  async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        "unauthenticated",
    ...
    if (!featureId || tokenCost === undefined) {
      throw new functions.https.HttpsError(
        "invalid-argument",
        "Feature ID and token cost are required.",
      );
    }

    try {
      const featureRef = admin
        .firestore()
...
export const getFeatureCosts = functions
  .region("us-central1")
  .https.onCall(async (data, context) => {
    try {
      const featuresSnapshot = await admin
        .firestore()
        .collection("featureCosts")
        .get();
 ...
 */
export const getReferrerInfoFunction = functions
  .region("us-central1")
  .https.onCall(async (data, context) => {
    const { referralCode } = data;
    if (!referralCode) {
  ...
  switch (event.type) {
    case "checkout.session.completed":
      ...
      if (session.metadata && session.metadata.firebaseUID) {
        ...
        console.log(
          `Processing checkout session for user: ${uid}, customer: ${customerId} `,
        );
...
export const onProjectUpdated = functions.firestore
  ...
  .onUpdate(async (change, context) => {
    ...
    if (afterVersions.length > beforeVersions.length) {
      ...
      console.log(
        `New version detected for project ${projectId}. Version: ${newVersion.versionNumber}`,
      );
 ...
 */
export const adminTriggerDigest = functions.https.onCall(
  async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        "unauthenticated",
    ...
    if (!callerDoc.data()?.isAdmin) {
      throw new functions.https.HttpsError(
        "permission-denied",
        "Must be an admin to trigger digests.",
      );
    }
    ...
    try {
      ...
      return {
        success: true,
        message: `Successfully triggered ${frequency} digest.`,
      };
    } catch (error: any) {
...
export const fixProjectVersionCategory = functions.https.onCall(
  async (data, context) => {
    ...
    if (!projectName || !newCategory) {
      throw new functions.https.HttpsError(
        "invalid-argument",
        "projectName and newCategory are required",
      );
    }
...
} from "./engagement/engagementScore";
