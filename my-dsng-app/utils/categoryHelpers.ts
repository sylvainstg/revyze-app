...
export function getCategories(versions: ProjectVersion[]): string[] {
    const categories = new Set(versions.map(v => v.category || DEFAULT_CATEGORY));
...
export function getNextCategoryVersion(
    ...
    const maxVersion = Math.max(...categoryVersions.map(v => v.categoryVersionNumber));
    return maxVersion + 1;
}
 ...
 */
export function getCategoryCounts(versions: ProjectVersion[]): Record<string, number> {
 ...
 */
export function migrateVersionsToCategories(versions: ProjectVersion[]): ProjectVersion[] {
